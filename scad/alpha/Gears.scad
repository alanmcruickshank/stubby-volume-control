// Based on Work by Greg Frost.

// Additions by Alan Cruickshank 2017
...
// for print
union() {
    translate ([-(15),0,0]) volume_planet_gear();

    translate ([-45,-25,0]) volume_planet_gear();
    translate ([-70,-25,0]) volume_planet_gear();
    translate ([-45,25,0]) volume_planet_gear();
    translate ([-70,25,0]) volume_planet_gear();

    translate ([12.5,0,0]) upper_sun_gear();
    translate ([-55,0,0]) lower_sun_gear();

    volume_ring_gear(); 
}

module volume_planet_gear(){
    scale([0.5,0.5,0.5]) translate ([0,0,10]) plain_double_helix_gear (teeth=9,height=20,bore_diameter=6.6, clearance=0.5, backlash=1);
}

module volume_ring_gear(){
    scale([0.5,0.5,0.5]) translate ([0,0,10]) ring_gear(clearance=0.5, backlash=1, knurling=3); 
}

module upper_sun_gear(){
    scale([0.5,0.5,0.5]) half_fixed_gear(clearance=0.5, backlash=1.5);
}

module lower_sun_gear(){
    translate ([0,0,5]) scale([0.5,0.5,0.5]) mirror([0,0,1]) half_fixed_gear(clearance=0.5, backlash=1);
}


module half_fixed_gear(clearance=0.2, backlash=0){
    height = 10;
    teeth = 13;
    difference(){
        mirror ([0,1,0])
            gear (number_of_teeth=teeth,
                ...
                twist=200/teeth);
        for (i = [0:4]){
            rotate([0,0,i*72]) translate([12 + 4, 0, 0]) cylinder(h=height, d=6.4);
        }
    }
}


module ring_gear(teeth=31, gear_height=10, ring_width=5, clearance = 0.2, backlash=0, knurling=0) {
    circular_pitch = 700;
    ...
    twist=200/number_of_teeth;
    involute_facets=0;
    
    // use same pitch for knurling as for teeth

    // For a ring gear we need to swap the addendum and dedendum
    ring_outer_radius = pitch_radius+dedendum;
    ring_root_radius = pitch_radius-addendum;

    full_outer_radius=outer_radius + ring_width;
    
    difference(){
        union(){
            difference(){
                cylinder(h=gear_height, r=full_outer_radius);
                union(){
                    translate([0,0,-1]) cylinder(h=gear_height+2, r=root_radius);
                    for (i = [0:number_of_teeth]){
                        rotate([0,0,i * (360/number_of_teeth)])
                        linear_extrude (height=gear_height, convexity=10, twist=twist)
                        involute_gear_tooth (
                            pitch_radius,
                            ring_root_radius,
                            base_radius,
        ...
        if (knurling>0) {
            for (i = [0:number_of_teeth]){
                rotate([0,0,i * (360/number_of_teeth)])
                    translate([full_outer_radius,0,-1])
                        cylinder(r=knurling, h=gear_height*2+2, center=true);
                        //cube([knurling, 4, gear_height*2], center=true);
            }    
        }
    }
}
...
module plain_double_helix_gear (
	teeth=17,
    height=20,
    bore_diameter=5,
    clearance = 0.2,
